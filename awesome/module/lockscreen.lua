...
local default_wall_name = 'scifi.jpg'						-- Default wallpaper
...
local locker = function(s)
	...
	local lockscreen = wibox {
		...
		ontop = true,
		type = "splash",
		width = dpi(s.geometry.width - 1000),
		height = dpi(s.geometry.height - 250),
		x = dpi(s.geometry.x - 1000),
		y = dpi(s.geometry.y - 250),
	...
	show_lockscreen = function()
		...
		-- Hide start menu if open to not interfere with keygrabber
		for s in screen do
...
local locker_ext = function(s)
	local extended_lockscreen = wibox {
		...
		x = dpi(s.geometry.x),
		y = dpi(s.geometry.y),
		width = dpi(s.geometry.width - 960),
